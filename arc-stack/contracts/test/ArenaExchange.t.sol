...
contract ArenaExchangeTest is Test {
    ...
    function testCompleteLifecycleConservesCollateral() external {
        ...
        ArenaExchange.Order memory buy = _order(buyer, true, 1, 600_000, uint128(10 * UNIT), 1, "buy-1");
        ArenaExchange.Order memory sell = _order(seller, false, 1, 550_000, uint128(10 * UNIT), 2, "sell-1");
        ...
        matches_[0] = ArenaExchange.Match({
            buyOrderHash: buyHash, sellOrderHash: sellHash, quantity: uint128(10 * UNIT)
        });
        vm.prank(matcher);
    ...
    function testRejectsReplayAndInvalidSignature() external {
        ArenaExchange.Order memory buy = _order(buyer, true, 0, 500_000, uint128(UNIT), 7, "buy-replay");
        ...
        ArenaExchange.Order memory forged = _order(buyer, true, 0, 500_000, uint128(UNIT), 8, "forged");
    ...
    function testPauseStopsRiskCreationButAllowsWithdrawalAndCancellation() external {
        ArenaExchange.Order memory buy = _order(buyer, true, 0, 500_000, uint128(UNIT), 9, "buy-pause");
        bytes32 orderHash = exchange.hashOrder(buy);
        exchange.submitOrder(buy, _signature(buyerKey, orderHash));
        exchange.pause();

        ArenaExchange.Order memory blocked = _order(buyer, true, 0, 500_000, uint128(UNIT), 10, "blocked");
