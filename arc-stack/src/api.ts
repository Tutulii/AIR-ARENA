...
export async function buildApi(dependencies: ApiDependencies) {
  ...
  app.get<{ Querystring: { status?: string; limit?: string } }>("/v1/markets", async (request) => {
    const limit = Math.min(100, Math.max(1, Number(request.query.limit ?? 50) || 50));
    const values: unknown[] = [];
    let where = "";
    if (request.query.status) {
      values.push(request.query.status.toUpperCase());
    ...
    try {
      ...
      await client.query(
        `INSERT INTO arc_markets(
         ...
         ) VALUES ($1,$2,$3,$4,$5,'QUEUED','TXLINE_1X2_REGULATION')
