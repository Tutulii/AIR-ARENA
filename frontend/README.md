...
MCP access while using the separate AIR Arena API, MCP, and settlement runtime.
...
`NEXT_PUBLIC_API_URL` must point to the AIR Arena API deployment with SPORT and
TxLINE routes enabled. The deployed API must allow the AIR Arena frontend domain
through its `CORS_ORIGINS` configuration.
